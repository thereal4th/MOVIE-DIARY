...
export const DualPaneNotebook: React.FC<DualPaneNotebookProps> = ({ leftPage, rightPage }) => {
  return (
    <div className="flex flex-col lg:flex-row items-stretch h-full w-full min-h-0 overflow-hidden divide-y-2 lg:divide-y-0 divide-[var(--border-color)]">
      ...
      <div className="flex-1 min-h-0 min-w-0 flex flex-col w-full lg:w-[calc(50%-24px)] h-full overflow-hidden px-4 sm:px-6 lg:px-8 py-4 sm:py-6 bg-white/40 dark:bg-black/20 z-10">
      ...
      <div className="hidden lg:flex w-12 shrink-0 h-full center-ring-spine flex-col justify-between py-6 items-center z-20 select-none border-x border-[var(--border-color)]">
        {Array.from({ length: 14 }).map((_, i) => (
          <span key={i} className="binder-ring my-1.5 shadow-md transform hover:scale-105 transition-transform" />
        ))}
      </div>
